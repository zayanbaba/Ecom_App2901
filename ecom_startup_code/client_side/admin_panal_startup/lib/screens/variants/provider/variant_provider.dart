    ...
    try {
      ...
      if (response.isOk) {
        ...
        if (apiResponse.success == true) {
          clearFields();
          SnackBarHelper.showSuccessSnackBar(apiResponse.message ?? 'Variant added successfully');
          log('Variant added');
    ...
    try {
      if (variantForUpdate != null) {
        ...
        if (response.isOk) {
          ...
          if (apiResponse.success == true) {
            clearFields();
            SnackBarHelper.showSuccessSnackBar(apiResponse.message ?? 'Variant updated successfully');
            log('Variant updated');
            _dataProvider.getAllVariant();
          } else {
            SnackBarHelper.showErrorSnackBar('Failed to update Variant: ${apiResponse.message}');
          }
        } else {
          SnackBarHelper.showErrorSnackBar(
            'Error: ${response.body?['message'] ?? response.statusText}',
      ...
      if (response.isOk) {
        ...
        if (apiResponse.success == true) {
          SnackBarHelper.showSuccessSnackBar(
            apiResponse.message ?? 'Variant deleted successfully',
          );
          _dataProvider.getAllVariant();
        } else {
          SnackBarHelper.showErrorSnackBar(
